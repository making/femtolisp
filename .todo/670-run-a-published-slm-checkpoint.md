...
| B-2 | `480`'s remainder -- the `--simd` GEMV accumulator chain, audited to a close | Medium | **CLOSED 2026-09-10, as an audit that turned up a defect.** All three open bullets answered on GB10: `Solo.java`'s numbers taken (the clean per-process harness reproduces dorian's SIGN at 48 columns, 0.96x Graal / 0.89x C2, against `Gate.java`'s 1.21x -- so the disagreement is the harness's, not the box's, and from 96 columns up both JITs win 1.12-1.77x); the head-dimension bullet audited green (`489`'s Qwen3.5-0.8B at `head_dim` 128, then `728`); and columns 16-31 pinned at 24 and 31 on all four `--simd` implementations plus a corpus case, which is where the region stops being untested. **The gate was re-decided on a model rather than left on the probe**: three builds of one tree differing only in `MATVEC_ACC_THRESHOLD` decode stories15M at 343.9 (gate off) / 393.1 (32, shipped) / 402.6 (96) tok/s, so the shipped gate is worth 1.13x and moving it to 96 is 1.001x with the spread across 1.0 -- it stays at 32, now for a measured reason. What the audit found is `.todo/758`: the four `--simd` backends do NOT agree on an f32 reduction whose length is not a multiple of 4 (wasm-GC folds a zero-padded final group, the other three add a scalar tail), a contract `.kb/vec.md` states as holding and every cross-backend probe was on one side of. Not claimed for a lane -- it is the `vec:` reduction contract, not this umbrella's subject (rule 15). Also unlanded here: the stale `.kb/bfloat16.md` line below, handed to A under rule 14 |
