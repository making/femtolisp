...
[begin, end] }, ... }` です。どちらのリーダーもファイルを先頭から順に歩き（rontolisp
のストリームは位置を変えられません。`file-position` は `nil` を返します）、要求され
なかったものは `checkpoint:skip-bytes` で読み飛ばします。
...
**テンソルを飛ばして節約できるのはメモリであって I/O ではありません。**`gguf:read`
の `:only` はテンソル名のリスト、`safetensors:read` の `:only` は名前に対する述語です。
どちらでも要らないテンソルはステージングも変換もされず有界の読み取りで読み飛ばされる
ので、マルチモーダルなチェックポイントの視覚タワーは読むバイト数だけを払います --
ただしそれは払います。ストリームはそこを飛び越えられないからです。
