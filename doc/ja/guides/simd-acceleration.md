...
`#bf16` は*格納用*の幅であり、`--simd` もそのように扱います。チェックポイントの重みが実際に読まれる形だけをアクセラレートし、それ以外は移植版の `vec.lisp` の定義に任せます -- 答えは同じで、速くならないだけです。対象は 3 つのメンバだけで、インタプリタと JVM に限られます(WASM バックエンドにはそもそも bfloat16 配列がありません)。
...
この組み合わせは意図的に狭くしてあります。bfloat16 の重みに対する単精度の活性化 -- 公開されたチェックポイントがデコードされるときの組み合わせであり、カーネルを持つ唯一の組み合わせです。それ以外のオペランドの組み合わせはすべて移植版の定義が計算します: `#bf16` 同士、`#bf16` と `#d`、そしてこの幅での要素ごとの呼び出し(`vec:add`・`vec:mul`・`vec:relu`、`-into` の兄弟も含めてすべて)。幅の混ざった要素ごとの呼び出しも拒否されるのではなく計算されます -- `--simd` が答えをエラーに変えることはありません。
