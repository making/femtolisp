...
要素ごとの行も、小さな規模での同じ考えです。両方のオペランドをレーンループの内側で拡張し、単精度で計算して、格納時に狭めます -- 狭い幅で中間値を保持することはありません。狭める処理は分岐なしのレーン形式で(複合カーネルの実測ではスカラー経路の 2.3-2.7 倍)、単精度の中間値は移植版の定義の答えとビット単位で一致します。ミラーされるのは単精度のレーンプーループを持つメンバだけです。`vec:scale` は真の倍精度スカラーを掛け、比較セレクトはスカラーループで、超越単項関数は要素ごとに `java.lang.Math` を呼びます -- いずれもミラーすべきレーン形式を持ちません。
