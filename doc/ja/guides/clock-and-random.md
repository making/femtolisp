...
`--optimize`(`off` 以外のどのレベルでも)では、プログラムが実際に draw できるときだけこのフックがエクスポートされます。`random`
を一度も呼ばないモジュールにはシードすべきものがないので、フックの本体も名前も残りません — 無条件に呼ぶのではなく、存在を確かめてください:
...
シードフックと同じ規則が適用されます: `--optimize` では時計を読まないモジュールは `__ronto_set_time`
をエクスポートしません。自分でビルドしていないモジュールを扱うホストなら `?.` 経由で呼んでください。
