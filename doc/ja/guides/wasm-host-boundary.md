...
- `:string`/`:s-expr` の**引数**は、モジュールのエクスポートする `memory` 内への `(ptr, len)` ペアとしてホストに届きます(`:s-expr` 引数は先に読み取り可能なテキストへ印字されます)。1 つのインポートに複数あってもそれぞれが専用の領域を持ち、呼び出しの間はすべて生存します — 復帰時に解放されるため、ホストは応答する前に必要な引数をすべて読み取る必要があります。
- **このポインタは借用された読み取り専用のものです。** そこへ書き込むことは未定義であり、何を壊すかは出力形状によって異なります: ここ(wasm-GC)ではその領域はこの 1 回の呼び出し用にラッパーがステージするスクラッチで、復帰時にポップされるため、呼び出し中に書き込んでも直後には死んでいるメモリに触れるだけです — 契約によってではなく、たまたま無害なだけです。[`--no-gc`](wasm-nogc.md#host-imports-rontolispwasm-import) にはステージングが一切なく、ポインタはモジュールがすでに保持しているブロックの `(ptr, len)` で、インスタンスの寿命いっぱい生存します。そのため書き込みはそのブロックを恒久的に破壊し、同じ綴りの文字列リテラルは 1 つのブロックへ重複排除されているため、その綴りを共有するすべての呼び出し箇所が、インスタンスが生きている限り道連れで壊れます。
