...
リアクターモジュール(`--no-wasi` または `--no-gc`)は、プログラムがホスト関数を宣言しない限り何もインポートしないため、ホスト側は丸ごと「インスタンス化してからエクスポートを呼び出す」だけです — そして Node とブラウザで同じコードです。端から端まで、コピー＆ペーストで動く完全な例を示します。3 つのエクスポートからなる小さなキットから始めます:
