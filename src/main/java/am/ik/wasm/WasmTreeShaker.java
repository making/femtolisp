...
public final class WasmTreeShaker {
	...
	/**
	 * An export that exists only to let a HOST write one linear-memory cell, kept only
	 * while some other surviving function still reads that cell.
	 * <p>
	 * A module with no imports cannot ask its host for entropy or for the time, so it
	 * exports a setter instead and the host calls it before the module runs. The setter
	 * is an export, hence a root, hence immortal -- and on a module that never draws a
	 * random number or reads a clock it writes a cell nothing will ever read. Its name,
	 * its body, its function-section entry and its export entry are then pure overhead
	 * (65 bytes on a 1.7 KB module, 4%).
	 * <p>
	 * Whether the cell is read is decided by OBSERVATION, in the sense of
	 * {@link DroppableDataRange}: the hook is kept when a function that survives the
	 * shake WITHOUT it -- any function but the hook's own body, which necessarily names
	 * the cell it writes -- holds an {@code i32.const} equal to the address. A
	 * linear-memory reference is an indistinguishable {@code i32.const}, so an unrelated
	 * constant that happens to equal the address only KEEPS the hook, never drops one a
	 * reader needs. The reader need not be a call away from the hook, and usually is not:
	 * nothing calls a setter, the cell is the whole channel.
	 *
	 * @param exportName the export's name
	 * @param cellAddress the linear-memory address the hook writes
	 */
	public record HostCellHook(String exportName, int cellAddress) {
	}
	...
	public static byte[] shake(byte[] module, List<OwnedDataSegment> ownedDataSegments,
		...
		return shake(module, ownedDataSegments, droppableDataRanges, List.of());
	}

	/**
	 * {@link #shake(byte[], List, List)}, additionally dropping every
	 * {@link HostCellHook} export whose cell no survivor reads.
	 * @param module a core WASM module (the 8-byte header followed by sections)
	 * @param ownedDataSegments data segments to drop when their owning functions are all
	 * unreachable
	 * @param droppableDataRanges byte ranges within a data segment to cut out when no
	 * surviving body holds an {@code i32.const} addressing into them
	 * @param hostCellHooks host-setter exports to drop when nothing else reads the cell
	 * they write
	 * @return an equivalent module with dead functions, orphaned data, unreferenced
	 * types, unread globals and unread host hooks removed
	 */
	public static byte[] shake(byte[] module, List<OwnedDataSegment> ownedDataSegments,
			List<DroppableDataRange> droppableDataRanges, List<HostCellHook> hostCellHooks) {
		return shakeWithRemap(module, ownedDataSegments, droppableDataRanges, hostCellHooks).module();
	}
	...
	public static ShakeResult shakeWithRemap(byte[] module, List<OwnedDataSegment> ownedDataSegments,
			List<DroppableDataRange> droppableDataRanges) {
		return shakeWithRemap(module, ownedDataSegments, droppableDataRanges, List.of());
	}

	/**
	 * {@link #shake(byte[], List, List, List)}, additionally reporting the function
	 * renumbering.
	 * @param module a core WASM module (the 8-byte header followed by sections)
	 * @param ownedDataSegments data segments to drop when their owning functions are all
	 * unreachable
	...
	public static ShakeResult shakeWithRemap(byte[] module, List<OwnedDataSegment> ownedDataSegments,
			List<DroppableDataRange> droppableDataRanges, List<HostCellHook> hostCellHooks) {
		ShakeResult shaken = dropUnreachable(module, ownedDataSegments, droppableDataRanges, hostCellHooks);
		...
		ShakeResult cleaned = dropUnreachable(folded.module(), List.of(), List.of(), List.of());
	...
	private static ShakeResult dropUnreachable(byte[] module, List<OwnedDataSegment> ownedDataSegments,
		...
		boolean needConstants = !droppableDataRanges.isEmpty() || !hostCellHooks.isEmpty();
		...
		// Roots: exported functions plus an optional start function -- minus, for now,
		// the host-cell hooks, whose own reachability is what the module's other
		// survivors decide (see HostCellHook).
		int[] hookFuncs = hookFunctions(hostCellHooks, exportSec);
		boolean[] hookLive = new boolean[hostCellHooks.size()];
		boolean[] reachable;
		// A hook kept in one round can only widen the survivor set, so the fixpoint is
		// monotone and settles in at most one round per hook.
		while (true) {
			reachable = new boolean[totalFuncs];
			Deque<Integer> work = new ArrayDeque<>();
			for (int root : exportFuncRoots(exportSec)) {
				if (root >= 0 && root < totalFuncs && !reachable[root] && !suppressedHook(root, hookFuncs, hookLive)) {
					reachable[root] = true;
					work.push(root);
				}
			}
			...
			boolean grew = false;
			for (int i = 0; i < hostCellHooks.size(); i++) {
				if (!hookLive[i] && hookFuncs[i] >= 0 && cellRead(hostCellHooks.get(i).cellAddress(), hookFuncs[i],
						reachable, numImportedFuncs, bodyConstants)) {
					hookLive[i] = true;
					grew = true;
				}
			}
		...
		// The export entries the dead hooks own go with their bodies: an export naming a
		// dropped function is not a module.
		java.util.Set<String> droppedExports = new java.util.HashSet<>();
		for (int i = 0; i < hostCellHooks.size(); i++) {
			// A hook the module does not export (hookFuncs -1) is not a hook to drop:
			// the caller may offer the same list for every build shape.
			if (!hookLive[i] && hookFuncs[i] >= 0) {
				droppedExports.add(hostCellHooks.get(i).exportName());
			}
		}
		...
		// Globals the survivors still read or write, closed over the initializer
		// expressions of the globals that keeps alive.
		List<WasmSections.GlobalEntry> globalEntries = globalSec == null ? List.of()
				: WasmSections.scanGlobalEntries(globalSec.payload());
		...
		int totalGlobals = numImportedGlobals + globalEntries.size();
		boolean[] globalLive = markLiveGlobals(globalEntries, numImportedGlobals, totalGlobals, reachable,
				numImportedFuncs, bodyRefs, exportSec);
		int[] globalRemap = new int[totalGlobals];
		int nextGlobal = 0;
		for (int i = 0; i < totalGlobals; i++) {
			globalRemap[i] = globalLive[i] ? nextGlobal++ : -1;
		}
		...
		List<Ref> globalRefs = new ArrayList<>();
		for (int i = 0; i < globalEntries.size(); i++) {
			if (globalLive[numImportedGlobals + i]) {
				globalRefs.addAll(globalEntries.get(i).refs());
			}
		}
		...
		if (next == totalFuncs && nextType == totalTypes && nextGlobal == totalGlobals && deadSegments.isEmpty()
				&& deadRanges.isEmpty() && droppedExports.isEmpty()) {
		...
		for (Section s : sections) {
			switch (s.id()) {
				case SEC_TYPE -> addVector(rebuilt, SEC_TYPE,
						rebuildTypeSection(s.payload(), typeEntries, typeUsed, funcRemap, typeRemap, globalRemap));
				case SEC_IMPORT -> addVector(rebuilt, SEC_IMPORT,
						rebuildImports(imports, reachable, funcRemap, typeRemap, globalRemap));
				case SEC_FUNCTION -> addVector(rebuilt, SEC_FUNCTION,
						rebuildFunctionSection(defTypeIdx, numImportedFuncs, reachable, typeRemap));
				case SEC_GLOBAL -> addVector(rebuilt, SEC_GLOBAL, rebuildGlobalSection(s.payload(), globalEntries,
						globalLive, numImportedGlobals, funcRemap, typeRemap, globalRemap));
				case SEC_TAG ->
					addVector(rebuilt, SEC_TAG, applyRefs(s.payload(), tagRefs, funcRemap, typeRemap, globalRemap));
				case SEC_CODE -> addVector(rebuilt, SEC_CODE, rebuildCodeSection(codeEntries, bodyRefs,
						numImportedFuncs, reachable, funcRemap, typeRemap, globalRemap));
				case SEC_EXPORT -> addVector(rebuilt, SEC_EXPORT,
						WasmSections.rebuildExportSection(s.payload(), funcRemap, globalRemap, droppedExports));
				case SEC_START ->
	...
	private static byte[] applyRefs(byte[] buf, List<Ref> refs, int[] funcRemap, int[] typeRemap, int[] globalRemap) {
		if (refs.isEmpty()) {
			return buf;
		}
		...
		for (Ref r : refs) {
			...
			int[] remap = switch (r.kind()) {
				case FUNC -> funcRemap;
				case TYPE_U, TYPE_S -> typeRemap;
				case GLOBAL -> globalRemap;
			};
			...
			switch (r.kind()) {
				case FUNC, TYPE_U, GLOBAL -> WasmSections.writeU(out, remap[r.index()]);
	...
	// The function index each hook export names, or -1 when the module does not export
	// that name at all (nothing to decide).
	private static int[] hookFunctions(List<HostCellHook> hooks, @Nullable Section exportSec) {
		int[] funcs = new int[hooks.size()];
		java.util.Arrays.fill(funcs, -1);
		if (exportSec == null || hooks.isEmpty()) {
			return funcs;
		}
		...
		for (int i = 0; i < count; i++) {
			String name = WasmSections.readName(payload, p);
			...
			for (int h = 0; h < hooks.size(); h++) {
				if (hooks.get(h).exportName().equals(name)) {
					funcs[h] = index;
				}
			}
		}
	...
	// Whether this export root is a hook not yet shown to be needed, so this round of
	// the fixpoint must not root it.
	private static boolean suppressedHook(int funcIndex, int[] hookFuncs, boolean[] hookLive) {
		for (int i = 0; i < hookFuncs.length; i++) {
			if (hookFuncs[i] == funcIndex && !hookLive[i]) {
				return true;
			}
		}
		return false;
	...
	// Whether any reachable function OTHER than the hook's own body names the address.
	private static boolean cellRead(int address, int hookFunc, boolean[] reachable, int numImportedFuncs,
			List<@Nullable IntList> bodyConstants) {
		for (int i = 0; i < bodyConstants.size(); i++) {
			int funcIndex = numImportedFuncs + i;
			if (funcIndex == hookFunc || !reachable[funcIndex]) {
				continue;
			}
			@Nullable IntList constants = bodyConstants.get(i);
			if (constants != null && constants.contains(address)) {
				return true;
			}
		}
		return false;
	...
	// The globals the shaken module still needs: every imported one (dropping an import
	// changes the host contract, and its index is part of it), every exported one, every
	// one a SURVIVING body reads or writes, and -- transitively -- every one those
	// globals' initializer expressions name. Everything else is a definition nothing can
	// observe: the backends emit one global per top-level Lisp variable whether or not
	// the program has a reader, and a runtime whose functions the shake just dropped
	// leaves its own behind.
	private static boolean[] markLiveGlobals(List<WasmSections.GlobalEntry> globalEntries, int numImportedGlobals,
			int totalGlobals, boolean[] reachable, int numImportedFuncs, List<List<Ref>> bodyRefs,
			@Nullable Section exportSec) {
		boolean[] live = new boolean[totalGlobals];
		Deque<Integer> work = new ArrayDeque<>();
		for (int i = 0; i < numImportedGlobals; i++) {
			live[i] = true;
		}
		for (int root : exportGlobalRoots(exportSec)) {
			if (root >= 0 && root < totalGlobals && !live[root]) {
				live[root] = true;
				work.push(root);
			}
		}
		for (int i = 0; i < bodyRefs.size(); i++) {
			...
			for (Ref r : bodyRefs.get(i)) {
				if (r.kind() == RefKind.GLOBAL && r.index() >= 0 && r.index() < totalGlobals && !live[r.index()]) {
					live[r.index()] = true;
					work.push(r.index());
				}
			}
		...
		while (!work.isEmpty()) {
			int g = work.pop();
			int defIndex = g - numImportedGlobals;
			if (defIndex < 0 || defIndex >= globalEntries.size()) {
				continue; // imported global: its initializer lives on the host side
			}
			for (Ref r : globalEntries.get(defIndex).refs()) {
				if (r.kind() == RefKind.GLOBAL && r.index() >= 0 && r.index() < totalGlobals && !live[r.index()]) {
					live[r.index()] = true;
					work.push(r.index());
				}
	...
	private static byte[] rebuildGlobalSection(byte[] payload, List<WasmSections.GlobalEntry> entries,
			boolean[] globalLive, int numImportedGlobals, int[] funcRemap, int[] typeRemap, int[] globalRemap) {
		List<byte[]> kept = new ArrayList<>();
		for (int i = 0; i < entries.size(); i++) {
			if (!globalLive[numImportedGlobals + i]) {
				continue;
			}
			WasmSections.GlobalEntry e = entries.get(i);
	...
	private static void seed(List<Ref> refs, boolean[] used, Deque<Integer> work, int totalTypes) {
		for (Ref r : refs) {
			if (r.kind() == RefKind.TYPE_U || r.kind() == RefKind.TYPE_S) {
				seedIndex(r.index(), used, work, totalTypes);
			}
		}
	}
	...
	private static byte[] rebuildTypeSection(byte[] payload, List<TypeEntry> entries, boolean[] typeUsed,
			int[] funcRemap, int[] typeRemap, int[] globalRemap) {
		List<byte[]> kept = new ArrayList<>();
	...
	private static byte[] rebuildImports(List<ImportEntry> imports, boolean[] reachable, int[] funcRemap,
		...
		for (ImportEntry e : kept) {
			WasmSections.writeRaw(body, applyRefs(e.raw(), e.refs(), funcRemap, typeRemap, globalRemap));
		}
		return body.toByteArray();
	}
	...
	private static byte[] rebuildCodeSection(List<byte[]> codeEntries, List<List<Ref>> bodyRefs, int numImportedFuncs,
		...
		for (int i = 0; i < codeEntries.size(); i++) {
			if (reachable[numImportedFuncs + i]) {
				kept.add(applyRefs(codeEntries.get(i), bodyRefs.get(i), funcRemap, typeRemap, globalRemap));
			}
		}
		WasmSections.writeU(body, kept.size());
		for (byte[] entry : kept) {
	...
	private static int[] exportGlobalRoots(@Nullable Section exportSec) {
		return exportRoots(exportSec, WasmSections.KIND_GLOBAL);
	}

	private static int[] exportFuncRoots(@Nullable Section exportSec) {
		return exportRoots(exportSec, WasmSections.KIND_FUNC);
	}

	// The indices the export section names for one descriptor kind: the module's roots
	// for that index space.
	private static int[] exportRoots(@Nullable Section exportSec, int wantedKind) {
		if (exportSec == null) {
			return new int[0];
		}
		byte[] payload = exportSec.payload();
